...
beforeAll(
  () =>
    new Promise((resolve) => {
      ...
      app.get('/api/v2/diagnostics/arp_table', (_req, res) =>
        res.json({
          data: [{ mac: 'aa:bb:cc:dd:ee:ff', ip: '10.0.0.42', hostname: 'tv', interface: 'lan' }],
        }),
      );
      app.get('/api/v2/interfaces', (_req, res) =>
        res.json({
          data: [
            { if: 'wan', descr: 'WAN' },
      ...
      app.get('/api/v2/status/interfaces', (_req, res) =>
        res.json({
          data: [
            { name: 'wan', inbytes: 100, outbytes: 50 },
            { name: 'lan', inbytes: 999, outbytes: 888 },
          ],
        }),
      );
      server = app.listen(0, () => {
